...
class custom_strategy_util:
    ...
    def implement_strategy(data, buy_sl_pct, buy_dates, ema_period, sell_sl_pct, longer_tf,\
        ...
        for i in range(len(data)):
            ...
            if (custom_strategy_util.can_buy(st[i-1], prices[i-1], st[i], prices[i],\
                                             ema[i]) and\
                  custom_strategy_util.get_signal_based_on_macd(macd, macd_signal,\
                                                          longer_tf['Date'], date[i]) == "BUY" and\
                  buy_allowed):
                ...
                i += 1
                signal_list[i] = "BUY"
                potential_sl =  open_prices[i] - st[i-1]
                buy_time = date[i]
                last_trade_ctr = i
                ...
                status_list[i] = "BUY"
                sl_price = st[i-1]
                target_price = open_prices[i] + 2 * (open_prices[i] - sl_price)
                num_buys += 1
